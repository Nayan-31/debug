...
const gameInviteService = {
  ...
  invitePlayer: async (senderId, receiverId) => {
    ...
    const existingInvite = await gameInviteRepository.findPendingInvite(senderId, receiverId);
    if (existingInvite) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'Game invitation already sent');
    }
